...
- For **Hostinger production** hosting actions (create DB, upload to `public_html`, sibling PHP apps, cache clear) use skill **`hostinger-mcp-deploy`** — WP-CLI alone is not enough on shared hosting without SSH/`wp` on the server.
